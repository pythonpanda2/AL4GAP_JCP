...
class activesample:
    """
    A python class to perform active learning over a given AIMD trajectory. Automatically picks training configurations for the GAP IP model
    """
    ...
    def __init__(self, exyzfile, nsample=10, nminclust=10, truncate=1):
        ...
        if not os.path.isfile(exyzfile.strip('.extxyz') + '.pdb'):
            write(exyzfile.strip('.extxyz') + '.pdb',
                  self.exyztrj, format='proteindatabank')
    ...
    def gen_cluster(self):
        ...
        """
        hdb = HDBSCAN(metric='precomputed', min_samples=self.nsample, min_cluster_size=self.nminclust).fit(
            self.distances)  # allow_single_cluster=True
        labels = hdb.labels_
        ...
        for index, clustnum in enumerate(labels):
            if clustnum != -1:
                if clustnum not in lclustdict.keys():
                    lclustdict[int(clustnum)] = []

                # clustnum -->  int(clustnum) to avoid error in json writing
                lclustdict[int(clustnum)].append(index)
